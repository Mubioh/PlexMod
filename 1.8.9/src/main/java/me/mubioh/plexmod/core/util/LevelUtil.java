...
public final class LevelUtil {
    ...
    public static LevelProgress getLevelProgress(long xp) {
        ...
        if (level >= 100) return new LevelProgress(100, xp, THRESHOLDS[100], -1, 100);
        long cur  = THRESHOLDS[level];
        long next = THRESHOLDS[level + 1];
        int  pct  = (int)(((xp - cur) * 100) / (next - cur));
        return new LevelProgress(level, xp, cur, next, pct);
    }

    public static class LevelProgress {
        public final int  level;
        public final long xp;
        public final long currentThreshold;
        public final long nextThreshold;
        public final int  pct;

        public LevelProgress(int level, long xp, long currentThreshold, long nextThreshold, int pct) {
            this.level            = level;
            this.xp               = xp;
