...
public final class PlayerDataService {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger("PlexMod/PlayerData");
    ...
    private static final Set<String> STAFF_GROUPS = Set.of(
    ...
    private PlayerDataService() {
        executor.scheduleWithFixedDelay(this::processBatch, CYCLE_DELAY_MS, CYCLE_DELAY_MS,
                TimeUnit.MILLISECONDS);
    }

    public boolean isLocalPlayerStaff() {
        return isStaff;
    }

    public void fetchLocalPlayerRank(String uuid) {
        Thread t = new Thread(() -> {
            try {
    ...
    private void processBatch() {
        ...
        for (String uuid : batch) {
            try {
                ...
                LOGGER.debug("Failed to fetch data for {}: {}", uuid, e.getMessage());
                cache.putIfAbsent(uuid, new CachedData(new PlayerData(0)));
            }
        }
    }
    ...
    private static long extractLong(String json, String key) {
        ...
        if (colon == -1) throw new NoSuchElementException("Colon not found after key");
        int start = colon + 1;
